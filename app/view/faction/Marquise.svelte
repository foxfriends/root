...
<div class='container' bind:clientWidth={width} bind:clientHeight={height}>
  <div class='board' style={`width: ${2252 * scale}px; height: ${1749 * scale}px`}>
    {#each sawmills as building, i (building.id)}
      <Building {building} x={tile.x - tile.dx * i} y={tile.y} {scale} />
    {/each}
    {#each workshops as building, i (building.id)}
      <Building {building} x={tile.x - tile.dx * i} y={tile.y + tile.dy} {scale} />
    {/each}
    {#each recruiters as building, i (building.id)}
      <Building {building} x={tile.x - tile.dx * i} y={tile.y + 2 * tile.dy} {scale} />
    {/each}
    <Token tokens={woods} x={wood.x} y={wood.y} {scale} />
