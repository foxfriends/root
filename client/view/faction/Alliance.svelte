...
<div class='container'>
  <div class='board' style={`width: ${2252 * scale}px; height: ${1749 * scale}px`}>
    {#if $game.factionData.alliance.bases.fox}
      <Token square image={tokenImages[Piece.alliance.base_fox.key]} x={base.x - base.dx * 2} y={base.y} {scale} />
    {/if}
    {#if $game.factionData.alliance.bases.rabbit}
      <Token square image={tokenImages[Piece.alliance.base_rabbit.key]} x={base.x - base.dx} y={base.y} {scale} />
    {/if}
    ...
    {#each new Array($game.factionData.alliance.sympathy).fill(0) as _, i}
      <Token round image={tokenImages[Piece.alliance.sympathy.key]} x={sympathy.x - i * sympathy.dx} y={sympathy.y + (i % 2) * sympathy.dy} {scale} />
    {/each}
