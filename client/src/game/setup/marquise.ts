...
export default async function* setupMarquise(this: Client) {
        ...
        game!.board.clearings
          .filter(
            (clearing) =>
              ...
              game!.board.paths.some(
                (path) =>
                  path.includes(clearing.index) &&
                  path.includes(keepClearing.index),
              ),
          )
  ...
  const workshopClearing = locate.call(
    get(game)!.board,
  ...
  const recruiterClearing = locate.call(
    get(game)!.board,
