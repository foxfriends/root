...
import Suit from "server/model/Suit.js";
...
export default async function* setupCult(this: Client) {
  if (!locate.call(get(game)!.board, Piece.cult.warrior)) {
    ...
    if ((keepClearing && eyrieClearing) || (!keepClearing && !eyrieClearing)) {
      ...
      prompts.set({
        ...
        clearings: get(game)!
          .board.clearings.filter((clearing) => clearing.isCorner)
          .filter(
            (clearing) =>
              !keepClearing || clearing.index !== keepClearing.index,
          )
          .filter(
            (clearing) =>
              !eyrieClearing || clearing.index !== eyrieClearing.index,
          ),
      });
      game.set(
  ...
  game.set(
    yield* accept.call(this, {
      type: "Prompts:outcast",
      async *handler({ outcast }: { outcast: Suit }) {
