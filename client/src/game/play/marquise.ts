  ...
  switch (get(game)!.phase) {
    ...
    case 1:
      const sawmillClearings = (<Clearing[]>[]).concat(
        ...get(game)!
          .board.clearings.filter((clearing) =>
            Clearing.hasBuilding(clearing, Pieces.marquise.sawmill),
          )
          .map((clearing) =>
            clearing.buildings
              .filter(
          ...
          game.set(
            yield* accept.call(this, {
              ...
              async *handler({ clearing }: { clearing: Clearing }) {
                const game = await this.send("clearing", {
                  clearing: clearing.index,
                });
                sawmillClearings.splice(
                  sawmillClearings.findIndex((c) => c.index === clearing.index),
                  1,
                );
                return game;
              },
