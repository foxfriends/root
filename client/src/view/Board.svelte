<script>
  ...
  const mapImages = {
    forest: "/image/map-forest.jpg",
    winter: "/image/map-winter.jpg",
  };
  ...
  $: pan = {
    x: Math.floor(
      Math.min(Math.max(0, targetPan.x), boardWidth * scale - tableWidth),
    ),
    y: Math.floor(
      Math.min(Math.max(0, targetPan.y), boardHeight * scale - tableHeight),
    ),
  };
...
<div class="container" on:wheel={zoom} on:mousemove={drag}>
  <div
    class="viewport"
