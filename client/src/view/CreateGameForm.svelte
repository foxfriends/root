<script>
  ...
  import GameMap from "server/model/GameMap.js";
  ...
  $: valid =
    ...
    !(
      factions.includes(Faction.marquise) &&
...
<button class="button back" on:click={() => dispatch("back")}>
  <Text text="back" />
</button>
<h1 class="heading"><Text text="game-name" /></h1>
<!-- TODO [l10n]: the placeholder is not localized -->
<input class="input" placeholder="Name" autofocus bind:value={name} />
...
<div class="options">
  <fieldset>
    ...
    {#each Object.values(Faction) as faction}
      <label>
        <input type="checkbox" bind:group={factions} value={faction} />
  ...
  <div class="flex">
    <fieldset>
      ...
      <label
        ><input type="radio" bind:group={assignment} value="auto" />
        <Text text="random" /></label
      >
      <label
    ...
    <fieldset>
      ...
      {#each Object.values(GameMap) as gameMap}
        <label
          ...
          <Text text={gameMap} /></label
        >
      {/each}
    </fieldset>
  </div>
</div>
<button class="button" disabled={!valid} on:click={create}>
  <Text text="create" />
</button>
