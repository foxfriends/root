<script>
  ...
  $: focusedIndex = $game.factions.indexOf(
    $game.players[focusedPlayer].faction,
  );

  function cardImage({ key }) {
    return `/image/card/card-shared-front.${key}.jpg`;
  }
  ...
  <div class="pager">
    <div class="pages" bind:clientWidth={width} bind:clientHeight={height}>
      {#each $game.factions as faction, i (faction)}
        <div
          class="page"
          style={`transform: translateX(${(i - focusedIndex) * 100}%); opacity: ${focusedIndex === i ? 1 : 0}`}
        >
          <FactionCard
            {faction}
            {client}
    ...
    <div class="previews">
      {#each $game.playerNames as name}
        <img
          ...
          src="/image/card-{$game.players[name].faction}-front.jpg"
