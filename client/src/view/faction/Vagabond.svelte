<script>
  ...
  import { Item } from "server/model/Item.js";
  import Token from "../Token.svelte";
  import Deck from "../Deck.svelte";

  function tokenImage(token) {
    return `/image/token/token.${token}.png`;
  }

  function characterImage(character) {
    return `/image/card/card-vagabond_character-front.${character}.jpg`;
  }
  ...
  $: satchelItems = [
    ...
    ...$game.factionData[faction].items.exhausted.map((item) => ({
      state: "exhausted",
      item,
    })),
  ];
  $: damagedItems = $game.factionData[faction].items.damaged;
  $: tea = $game.factionData[faction].items.refreshed.filter(
    (item) => item.name === Item.tea,
  ).length;
  $: coin = $game.factionData[faction].items.refreshed.filter(
    (item) => item.name === Item.coin,
  ).length;
  $: bag = $game.factionData[faction].items.refreshed.filter(
    (item) => item.name === Item.bag,
  ).length;
  $: relationshipFactions = Object.entries($game.factionData[faction].relations)
...
<div class="container">
  <div
    class={`board ${faction}`}
    style={`width: ${2252 * scale}px; height: ${1749 * scale}px`}
  >
    {#if $game.factionData[faction].character}
      ...
      >
        <Deck
          cardImage={characterImages[$game.factionData[faction].character]}
          cardCount={1}
        />
      </div>
    {/if}
      ...
      >
        {#each factions as faction, i}
          <div
            style={i === 0
              ? ""
              : `margin-top: ${Math.min(5, (394 - factions.length * 146) / (factions.length - 1))}px;`}
          >
            <Token
              square
              block
              image={tokenImages[`vagabond-relationship_${faction}`]}
            />
          </div>
        {/each}
      </div>
    ...
    >
      {#each hostileFactions as faction, i}
        <div
          style={i === 0
            ? ""
            : `margin-left: ${Math.min(5, (439 - hostileFactions.length * 146) / (hostileFactions.length - 1))}px;`}
        >
          <Token
            square
            block
      ...
      {#each satchelItems as itemInfo}
        <div class={`item ${itemInfo.state}`}>
          <Token
            square
            block
            image={tokenImages[itemInfo.item.key]}
            radius={31}
          />
        </div>
      {/each}
    ...
    >
      {#each damagedItems as item}
        <div
          ...
          style={i === 0
            ? ""
            : `margin-left: ${Math.min(5, (586 - damagedItems.length * 146) / (damagedItems.length - 1))}px;`}
        >
