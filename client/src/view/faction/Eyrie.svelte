<script>
  ...
  function leaderImage(card) {
    return `/image/card/card-eyrie_leader-front.${card}.jpg`;
  }
...
<div class="container">
  ...
  >
    {#each new Array($game.factionData.eyrie.roost).fill(0) as _, i}
      <Token
        square
        image="/image/token/token.{Piece.eyrie.roost.key}.png"
        x={tile.x - tile.dx * i}
        y={tile.y}
        {scale}
      />
    ...
    {#if $game.factionData.eyrie.leader}
      ...
      >
        <Deck
          cardImage={leaderImage($game.factionData.eyrie.leader)}
          cardCount={1}
        />
      </div>
    {/if}
    <CraftedItems
      {...craftedItems}
      {scale}
      items={$game.factionData.eyrie.craftedItems}
    />
  </div>
</div>
