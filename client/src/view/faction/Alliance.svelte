<script>
  ...
  function cardImage(card) {
    return `/image/card/card-shared-front.${card}.jpg`;
  }
...
<div class="container">
  ...
  >
    {#if $game.factionData.alliance.base_fox}
      <Piece
        piece={Pieces.alliance.base_fox}
        x={base.x - base.dx * 2}
        y={base.y}
        {scale}
      />
    ...
    {#if $game.factionData.alliance.base_rabbit}
      <Piece
        piece={Pieces.alliance.base_rabbit}
        x={base.x - base.dx}
        y={base.y}
        {scale}
      />
    ...
    {#each new Array($game.factionData.alliance.sympathy).fill(0) as _, i}
      <Piece
        piece={Pieces.alliance.sympathy}
        x={sympathy.x - i * sympathy.dx}
        y={sympathy.y + (i % 2) * sympathy.dy}
        {scale}
      />
    {/each}
    {#if $game.factionData.alliance.supporters.length}
      ...
      >
        {#if isMe}
          <Pile
            cards={$game.factionData.alliance.supporters.map((card) =>
              cardImage(card.key),
            )}
          />
        {:else}
          <Deck
            cardImage="/image/card/card-shared-back.jpg"
    ...
    <CraftedItems
      {...craftedItems}
      {scale}
      items={$game.factionData.alliance.craftedItems}
    />
  </div>
</div>
