<script>
  ...
  import Pieces, { Piece as PieceT } from "server/model/Piece.js";
  ...
  function intersects(placement, forces) {
    return forces.some(
      ({ x, y, r }) =>
        Math.abs(placement.x - x) < placement.r + r &&
  ...
  $: arrangedPieces = ((pieces) => {
    ...
    const arrangedPieces = pieces.map((piece, i) => {
      ...
      const netForce = effectiveForces
        ...
        .reduce(({ x: tx, y: ty }, { x, y }) => ({ x: tx + x, y: ty + y }), {
          x: 0,
          y: 0,
        });
      ...
      while (intersects(placement, effectiveForces)) {
        placement.x +=
          Math.max(1, Math.ceil((Math.abs(netForce.x) / forceMagnitude) * 35)) *
          Math.sign(netForce.x);
        placement.y +=
          Math.max(1, Math.ceil((Math.abs(netForce.y) / forceMagnitude) * 35)) *
          Math.sign(netForce.y);
      }
  ...
  function image(piece) {
    return `/image/token/token.${piece.key}.png`;
  }
</script>
