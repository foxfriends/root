...
import subset from "../../../util/subset.js";
...
import Pieces, { Piece } from "../../../model/Piece.js";
import Suit from "../../../model/Suit.js";
import { Item } from "../../../model/Item.js";
import Clearing from "../../../model/board/Clearing.js";
import { Card } from "../../../model/Card.js";
import { Cancel, NoPiecesOfFaction } from "./rejections.js";
import cardEffect from "./cardEffect.js";
import returnPiece from "./returnPiece.js";

export async function* birdsong(
  this: Client,
  faction: Faction,
): AsyncIterableIterator<void> {
  // TODO: check victory conditions
  if (
    this.game.factions.includes(Faction.riverfolk) &&
...
export async function* daylight(
  this: Client,
  faction: Faction,
): AsyncIterableIterator<void> {
  // TODO: activate crafted effects
...
export async function* craft(
  ...
  if (faction_ === Faction.marquise_bot) {
    return;
  }
  const faction = faction_;

  async function* craftCard(
    this: Client,
...
function strength(game: Game, clearing: Clearing, faction: Faction): number {
  ...
  if (game.services.mercenaries === faction) {
    units += clearing.pieces.filter((piece) =>
      Piece.equals(piece, Pieces.riverfolk.warrior),
    ).length;
  }

  if (faction === Faction.vagabond || faction === Faction.vagabond2) {
    units += [
      ...game.factionData[faction]!.items.refreshed,
      ...game.factionData[faction]!.items.exhausted,
    ].filter((item) => item.name === Item.sword).length;
    const allies = Object.entries(game.factionData[faction]!.relations)
      .filter(([, level]) => level === 3)
      .map(([faction]) => faction);
    units += clearing.pieces.filter(
      (piece) => piece.name === "warrior" && allies.includes(piece.faction!),
    ).length;
  } else {
    units += clearing.pieces.filter((piece) =>
      Piece.equals(piece, Pieces[faction].warrior),
    ).length;
  }
...
async function* takeHits(
  ...
  TAKING_HITS: for (let i = 0; i < hits; ++i) {
    if (faction === Faction.vagabond || faction === Faction.vagabond2) {
      const items = [
        ...this.game.factionData[faction]!.items.refreshed,
        ...this.game.factionData[faction]!.items.exhausted,
      ];
      if (items.length <= hits - i) {
        this.game.factionData[faction]!.destroyItem(0);
      } else {
        for (;;) {
          const { damage: toDamage }: { damage: number } =
            await this.game.sendTo(faction, "damage");
      ...
      for (const target of targets) {
        const warrior = clearing.pieces.findIndex((piece) =>
          Piece.equals(Pieces[target].warrior, piece),
        );
        if (warrior !== -1) {
          // warrior always goes first
          yield* returnPiece.call(
            this,
      ...
      if (myBuildings.length <= hits - i) {
        yield* returnPiece.call(
          this,
          clearing.destroyBuilding(myBuildings[0])!,
          clearing,
          fromFaction,
        );
      }
      for (;;) {
        const { destroy: toDestroy }: { destroy: number } =
          await this.game.sendTo(faction, "destroy", {
        ...
        yield* returnPiece.call(
          this,
          clearing.destroyBuilding(toDestroy)!,
          clearing,
          fromFaction,
        );
        break;
...
export async function* battle(
  ...
  do {
    if (
      defender !== Faction.marquise_bot &&
      attacker !== Faction.marquise_bot
    ) {
      if (
        this.game.factionData[defender]!.hand.some(
          (card) =>
            card.name === Card.ambush &&
            (card.suit === clearing.suit || card.suit === Suit.bird),
        )
      ) {
        ...
        if (index !== undefined) {
          ...
          if (
            ...
            this.game.discard(
              ...this.game.factionData[defender]!.hand.splice(index, 1),
            );
            if (
              this.game.factionData[attacker]!.hand.some(
                (card) =>
                  card.name === Card.ambush &&
                  (card.suit === clearing.suit || card.suit === Suit.bird),
              )
            ) {
              const { card: index } = await this.game.sendTo(
                attacker,
                "ambush",
              );
              if (index !== undefined) {
                ...
                if (
                  ...
                  this.game.discard(
                    ...this.game.factionData[attacker]!.hand.splice(index, 1),
                  );
                  break;
                }
              }
  ...
  if (
    attacker !== Faction.marquise_bot &&
    this.game.factionData[attacker]!.craftedEffects.some(
      (card) => card.name === Card.brutal_tactics,
    )
  ) {
  ...
  if (
    ...
    this.game.factionData.eyrie!.leader === Leader.commander
  ) {
    attack++;
  }
  if (
