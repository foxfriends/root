...
import { InvalidCardSuit, NoTargetsForBattle, Cancel } from "./rejections.js";
...
function sawmill(sawmillClearings: Clearing[]) {
  return async function* sawmill(
    this: Client,
    { clearing: index }: { clearing: number },
    threadId: string,
  ): AsyncIterableIterator<void> {
    const clearing = sawmillClearings.find(
      (clearing) => clearing.index === index,
    );
    if (!clearing) {
      throw new NoSawmill(threadId);
    }
    this.game.factionData.marquise!.placeWood(
      this.game,
      clearing.index,
      threadId,
    );
...
async function* marquiseBirdsong(this: Client) {
  switch (this.game.phase) {
    ...
    /* falls through */
    case 1:
      const sawmillClearings = (<Clearing[]>[]).concat(
        ...this.game.board.clearings
          .filter((clearing) => clearing.hasBuilding(Pieces.marquise.sawmill))
          .map((clearing) =>
            clearing.buildings
              .filter(
                (building) =>
                  building && Piece.equals(building, Pieces.marquise.sawmill),
              )
              .map(() => clearing),
          ),
      );
...
async function* battle(this: Client): AsyncIterableIterator<void> {
  const clearing: Clearing = yield* accept.call(
    this,
    cancel,
    async function* clearing(
      ...
      if (
        clearing.pieces.some((piece) =>
          Piece.equals(piece, Pieces.marquise.warrior),
        ) &&
  ...
  switch (this.game.phase) {
    ...
    default:
      ...
      while (this.game.phase < allowedActions) {
        ...
        this.game.nextPhase();
        if (
          this.game.phase === allowedActions &&
          this.game.factionData.marquise!.hand.some(
            (card) => card.suit === Suit.bird,
          )
        ) {
          if (
            EXTRA_ACTION === (yield* accept.call(this, extraAction, "done"))
          ) {
            ++allowedActions;
          }
        }
