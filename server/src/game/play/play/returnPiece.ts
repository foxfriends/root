...
export default async function* returnPiece(
  this: Client,
  piece: Piece,
  clearing: Clearing,
  remover: Faction,
  ...
  if (piece.faction) {
    if (
      Piece.equals(piece, Pieces.riverfolk.trade_post_fox) ||
    ...
    if (Piece.equals(piece, Pieces.marquise.warrior)) {
      if (
        this.game.factionData.marquise!.hand.some(
          (card) => card.suit === clearing.suit || card.suit === Suit.bird,
        )
      ) {
        const keepClearing = this.game.board.locate(Pieces.marquise.keep);
        if (keepClearing) {
          const marquisePlayer = Object.values(this.game.players).find(
            (player) => player.faction === Faction.marquise,
          )!;
          // @ts-ignore
