...
async function* craftItem(
  ...
  if (faction === Faction.marquise_bot) {
  ...
  if (
    ...
    this.game.factionData.eyrie!.leader !== Leader.builder
  ) {
...
async function* favor(
  ...
  for (const clearing of this.game.board.clearings.filter(
    (clearing) => clearing.suit === suit,
  )) {
    for (let i = clearing.pieces.length; i >= 0; --i) {
      if (
        clearing.pieces[i].faction &&
        clearing.pieces[i].faction !== faction
      ) {
...
export default async function* cardEffect(
  ...
  switch (card.name) {
    ...
    case Card.royal_claim:
      if (
        this.game.factionData[faction]!.craftedEffects.some(
          (c) => c.name === card.name,
        )
      ) {
