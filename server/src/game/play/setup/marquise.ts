...
async function* placeSawmill(
  this: Client,
  { clearing }: { clearing: number },
  threadId: string,
): AsyncIterableIterator<void> {
  const keepClearing = this.game.board.locate(Piece.marquise.keep)!;
  const distance = this.game.board.clearingDistance(
    clearing,
    keepClearing.index,
  );
  if (distance > 1) {
  ...
  this.game.factionData.marquise!.placeBuilding(
    this.game,
    clearing,
    Piece.marquise.sawmill,
    threadId,
  );
  this.respond(threadId, "update", this.game);
}
...
async function* placeWorkshop(
  ...
  this.game.factionData.marquise!.placeBuilding(
    this.game,
    clearing,
    Piece.marquise.workshop,
    threadId,
  );
  this.respond(threadId, "update", this.game);
}

async function* placeRecruiter(
  this: Client,
  { clearing }: { clearing: number },
  threadId: string,
): AsyncIterableIterator<void> {
