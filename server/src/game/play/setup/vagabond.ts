...
import Characters from "../../../model/Character.js";
...
export default async function* setupVagabond(
  ...
  yield* accept.call(this, {
    ...
    async *handler(
      this: Client,
      { character }: { character: keyof typeof Characters },
      threadId: string,
    ): AsyncIterableIterator<void> {
      if (!Characters[character]) {
        throw new Error(`There is no character called ${character}`);
      }
      this.game.factionData[faction]!.setCharacter(
        this.game,
  ...
  yield* accept.call(this, {
    ...
    async *handler(
      this: Client,
      { forest }: { forest: number },
      threadId: string,
    ): AsyncIterableIterator<void> {
      this.game.board.forests[forest].addPiece(
        this.game,
        Piece[faction].warrior,
      );
      this.respond(threadId, "update", this.game);
    },
  });
