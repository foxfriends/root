...
import Pieces from "../../../model/Piece.js";
import Suit from "../../../model/Suit.js";
import { InvalidStartClearing } from "../../../model/factionData/rejections.js";

async function* setCultClearing(
  this: Client,
  ...
  for (const clearing of adjacentClearings) {
    this.game.factionData.cult!.placeWarriors(
      this.game,
      clearing.index,
      1,
      threadId!,
...
async function* chooseOutcast(
  this: Client,
  { outcast }: { outcast: Suit },
  threadId: string,
): AsyncIterableIterator<void> {
  if (!Suit[outcast]) {
...
export default async function* setupCult(this: Client) {
  if (!this.game.board.locate(Pieces.cult.warrior)) {
    ...
    if ((keepClearing && eyrieClearing) || (!keepClearing && !eyrieClearing)) {
      ...
      yield* setCultClearing.call(
        this,
        (keepClearing || eyrieClearing)!.acrossCorner!,
      );
    }
  }
