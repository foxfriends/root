...
export default class Board {
  ...
  locate(piece: Piece): Clearing | undefined {
    return this.clearings.find(
      (clearing) =>
        clearing.pieces.some((p) => Piece.equals(p, piece)) ||
  ...
  adjacentClearings(clearingIndex: number): Clearing[] {
    return this.clearings
      .filter((clearing) =>
        this.paths.some(
          (path) =>
            path.includes(clearing.index) && path.includes(clearingIndex),
        ),
      )
  ...
  clearingDistance(start: number, end: number) {
    ...
    while (current !== null && current !== end) {
      ...
      current = distances
        ...
        .reduce(
          (distance: [number, number | null], node: [number, number]) =>
            distance[0] < node[0] ? distance : node,
          <[number, null]>[Infinity, null],
        )[1]!;
    }
