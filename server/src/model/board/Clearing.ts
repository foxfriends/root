...
export default class Clearing {
  ...
  static ruler(game: Game, faction: Faction, clearing: Clearing) {
    const hasGarden = clearing.buildings.some(
      (building) =>
        !!building &&
    ...
    clearing.pieces
      ...
      .filter(
        (faction) =>
          faction !== Faction.vagabond && faction !== Faction.vagabond2,
      )
      .map((f) =>
        game.services.mercenaries && f === Faction.riverfolk ? faction : f,
      )
    ...
    clearing.buildings
      ...
      .filter((faction): faction is Faction => faction !== null)
      .forEach((faction) => scores[faction]++);
    const best = <[Faction, number]>(
      Object.entries(scores).reduce((best, score) =>
        score[1] > best[1] ? score : best,
      )
    );
