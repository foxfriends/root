    ...
    this.socket.addEventListener("message", ({ data }: { data: any }) => {
      try {
        ...
        if (message.type === "reject" || message.type === "error") {
          ...
          const error =
            message.type === "reject"
              ? new Rejection(message.threadId, message.data, true)
              : new Error(message.data);
          const [, callback]: OptionalCallbacks =
            this.callbacks.get(message.threadId) || [];
          if (callback) {
            console.log(
              `Response received (${message.threadId}): ${message.type}`,
            );
            callback(error);
          } else {
            // notify watchers
      ...
      try {
        if (this.receivedMessages.length) {
          ...
          yield await new Promise((resolve, reject) =>
            this.watchers.push([resolve, reject]),
          );
        }
      } catch (e) {
        console.error(e);
