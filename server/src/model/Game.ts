...
import Cards, { Card } from "./Card.js";
import Items, { Item } from "./Item.js";
import Quests, { Quest } from "./Quest.js";
...
import Alliance from "./factionData/Alliance.js";
import Cult from "./factionData/Cult.js";
import Eyrie from "./factionData/Eyrie.js";
import Marquise from "./factionData/Marquise.js";
import MarquiseBot from "./factionData/MarquiseBot.js";
import Riverfolk from "./factionData/Riverfolk.js";
import Vagabond from "./factionData/Vagabond.js";
...
class UnsupportedSettings extends Error {
  constructor(settings: Settings) {
    super(
      `The requested game settings are not supported: ${JSON.stringify(settings)}`,
    );
  }
}
...
export default class Game {
  ...
  constructor(
    ...
    this.factionData = <any>factions // casting because this is cool
      ...
      .reduce(
        (collection, faction) =>
          Object.assign(collection, { [faction.faction]: faction }),
        {},
      );
    ...
    this.cards = shuffle(
      factions.includes(Faction.marquise_bot)
  ...
  async sendTo(faction: Faction, message: string, data?: any) {
    const { username } = Object.values(this.players).find(
  ...
  setFaction(client: Client, faction: Faction, threadId: string) {
    ...
    const takenBy = Object.values(this.players).find(
      (player) => player.faction === faction,
    );
    if (takenBy) {
  ...
  rollDice(): [number, number] {
    this.dice = [Math.floor(Math.random() * 4), Math.floor(Math.random() * 4)];
    this.dice.sort();
    this.notify();
    return this.dice;
  }
  ...
  get isFull() {
    return (
      ...
      this.factions.filter((faction) => faction !== Faction.marquise_bot).length
    );
  }

  get allReady() {
    return (
      Object.values(this.players).every((player) => player.ready) && this.isFull
    );
  }
